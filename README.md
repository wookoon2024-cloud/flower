...
![Release](https://img.shields.io/badge/Release-v2.0.1-10B981?style=for-the-badge)
...
*업무 피로와 스트레스 케어, 6대 품종(히든 전설 포함) 육성, 112종 업적 도감, 화원 상점 & 반려동물 동행(고양이/강아지/토끼), 범정부 AI 및 OpenAI/로컬 LLM 완벽 연동*
...
| :---: | :---: | :---: | :---: | :---: |
| 🚀 **단독 실행파일 (.exe)** | **`v2.0.1`** | **`2026-08-20`** | `마음지킴이_실행파일_v2.0.1.zip` | [**⚡ 마음지킴이.exe 즉시 다운로드 (ZIP)**](https://github.com/wookoon2024-cloud/flower/raw/main/%EB%A7%88%EC%9D%8C%EC%A7%80%ED%82%B4%EC%9D%B4_%EC%8B%A4%ED%96%89%ED%8C%8C%EC%9D%BC_v2.0.1.zip) |
| 💻 **순수 소스코드 (.py)** | **`v2.0.1`** | **`2026-08-20`** | `마음지킴이_소스_v2.0.1.zip` | [**📦 소스코드 패키지 다운로드 (ZIP)**](https://github.com/wookoon2024-cloud/flower/raw/main/%EB%A7%88%EC%9D%8C%EC%A7%80%ED%82%B4%EC%9D%B4_%EC%86%8C%EC%8A%A4_v2.0.1.zip) |

> 💡 **설치 없이 즉시 실행:** `마음지킴이_실행파일_v2.0.1.zip` 압축 해제 후 **`마음지킴이.exe`를 더블클릭**하면 파이썬 설치 없이 즉시 실행됩니다!  
> 🔒 **보안망/개발자용:** 파이썬 소스로 직접 실행하시려면 `마음지킴이_소스_v2.0.1.zip` 압축 해제 후 `python main.py`를 실행하세요.
...
| :---: | :---: | :--- |
| **`v2.0.1`** | 2026-08-20 | • **🪴 기본 무료 도자기 받침대(Basic Saucer) 즉시 지급 및 장착 오류 완벽 해결**<br>• **🛡️ SQLite `user_inventory` UNIQUE 제약 조건 충돌 방지:** 기존 버전과의 호환 마이그레이션 로직을 보강하여 DB 초기화 시 `IntegrityError` 예외가 발생하지 않도록 조치<br>• **🌸 품종 도감(Species Catalog) 2열 카드 레이아웃 전면 리팩토링:** 좌측 아바타 프레임, 중앙 품종명/상태 뱃지 수평 정렬, 균형 잡힌 카드 높이(76px)로 대칭과 시인성을 대폭 강화<br>• **🛡️ 백그라운드 시그널 & 마스터 애니메이션 루프 예외 보호 강화:** 창 닫힘 시그널 해제 및 `_on_master_anim_tick` 전역 예외 처리로 비동기 안정성 확보 |
| **`v2.0.0`** | 2026-08-20 | • **🏪 화원 상점(Shop), 씨앗 주화(Coin) 경제, 5종 화분 받침대 & 5종 살아숨쉬는 반려동물(고양이/강아지/토끼) 동행 시스템 전격 오픈**<br>• **🪙 씨앗 주화 경제 시스템:** 112종 업적 달성 시마다 **+50 주화**, 매일 첫 포춘 쿠키 뽑기 시 **+20 주화** 적립! 기존 사용자도 달성 업적 수에 따라 자동으로 소급 지급<br>• **🪵 5종 프리미엄 화분 받침대 컬렉션:** 따스한 내추럴 원목(100🪙), 순백의 로열 대리석(200🪙), 황실의 찬란한 황금(350🪙), 신비로운 자수정 크리스탈(500🪙), 환상의 오로라 레인보우(750🪙) 받침대를 구입하여 화분 밑동에 우아하게 장착<br>• **🐾 5종 귀염뽀짝 반려동물 입양 & 자율 행동 시스템:** 삼색 아기냥이(250🪙), 턱시도 고양이(300🪙), 똘망 시바견(350🪙), 듬직한 리트리버(450🪙), 앙증맞은 흰토끼(300🪙) 입양! 화분 주변을 아장아장 산책하고, 꼬리를 살랑살랑 흔들며 앉아있고, 식빵 굽거나 zZZ 쿨쿨 낮잠 자고, 잠시 신나게 놀러갔다 하트와 함께 돌아오는 풀 애니메이션 탑재<br>• **🐾 반려동물 터치 교감 인터랙션:** 반려동물을 마우스로 콕 클릭하면 깡총 점프/애교 하트 파티클과 함께 고유 음성 말풍선 발화 & 화분 애정도(+3) 상승<br>• **🏷️ 화원 도감 탭 제목 간결화 & 750px 광폭 레이아웃:** 탭 제목을 `[🪴 화원] [🌸 품종] [🏪 상점] [🏆 업적] [📊 마음날씨] [🥠 포춘]`으로 간결하게 단축하여 좌우 스크롤 화살표(`< >`) 없이 6개 탭이 한눈에 쾌적하게 렌더링되도록 개선 |
